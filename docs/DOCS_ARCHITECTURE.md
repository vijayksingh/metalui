...
| 7 | `contracts` | Behaviour is written once, implemented twice | The pressable keyboard driving Segmented in React and the Swift capture side by side | Keys, focus, dismissal and names as a contract in `agent.md`; what Base UI gives; what Swift must match | every `keyboard` section, `/start/agents` | Segmented, Menu, Dialog |
...
**related.** Segmented (latched choice instead of an action), IconButton (icon-only), Toggle (on/off); Patterns: dialog-footer, confirm-destructive; Learn: 4 Motion has mass, 3 Recipes.
