...
| 18 | Button refit: compact 28 variant | 04 §1 | shipped | see `feat(components): compact button and token refit` | React `Button` gains `size: 'default'\|'compact'` (28, 12 pt, 14 glyph, raise-sm, ink2 until hover); every button value now comes from tokens.json `button` (`--mu-button-*`), press 50 ms linear. Swift `MetalButton(_:cap:size:action:)`, `MetalButtonStyle(cap:size:)`, `MetalButtonSize`, public `MetalButtonMetrics` (was internal); labels no longer truncate. |
