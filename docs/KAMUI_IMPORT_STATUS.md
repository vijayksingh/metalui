...
| 17 | Tooltip and popover / correction menu | 04 §8, §18 | tooltip shipped; menu pending | tooltip: see `feat(components): tooltip` | Tooltip: React `Tooltip` (`label`, `shortcut`, `side`, `open`), `TooltipProvider` (Base UI Tooltip; the toolbar now uses it); CSS `--mu-tooltip-*`; Swift `.metalTooltip(_:shortcut:edge:)` (MetalToolButton uses it), `.metalTooltipChip(_:shortcut:)`. Menu / correction popover: pending. |
