...
| 7a | Segmented control | shipped | see `feat(components): segmented control` | React `Segmented` (`options`, `value`, `defaultValue`, `onValueChange`, `size: 'compact'\|'regular'`, `aria-label`; Base UI RadioGroup); CSS `--mu-segmented-*`; Swift `MetalSegmented(_:selection:options:size:)`. |
| 7b | Lens bar | pending | | |
