...
| 4 | Hover engraving | shipped | see `feat(objects): hover engraving` | React `HoverEngraving` (`kind`, `details`, `tags`, `status: { led: 'live'\|'waiting'\|'failed'\|'off', text }`, `placement`, `open`, `immediate`); CSS `--mu-engraving-*`; Swift `MetalHoverEngraving`, `.metalHoverEngraving(_:placement:isPresented:)`, `MetalEngraving`. Derived to confirm: graphite engraving fill and emphasis. |
