...
- **`MetalFilterBar`** (rename `MetalLensBar`, keep a typealias; `Blocks/MetalFilterBar.swift`) is a composition block: `MetalSurface(.frost, radius: .pill)` › `MetalGlyph` (new primitive: 14 / 16 in ink, ink2, ink3) + `MetalLabel(.query)` + `MetalLabel(.engraved)` count + `MetalLabel(.engraved)` note with a waiting `MetalLed` when pending + `MetalSwitcher(.compact)` views + `MetalIconButton(.ghost)` pin and close. The source is now a `note: (text, pending)` the host writes; `MetalLensBarMetrics.height` is 38 (the reference) and `noteLedGap` 5 is new.
...
- **`MetalTimeScrubber` placement and ticks:** add generated placement keys for left 22 and bottom 5, a maximum of six earlier day labels plus TODAY, and the 4–96 % label clamp. The Swift composition currently derives its placement from spacing tokens and lays TODAY at the track end. Demo `style.css:400-412` (`#scrub`, `.sc-days`) and `app.js:979-1007` (`timeSpan`, `renderScrub`).
- **Past-world filter:** provide a generated material/filter recipe for the canvas world in Memory mode: CSS `sepia(.12) saturate(.85)`. The native scene temporarily uses local values with Core Image filters. Demo `style.css:497` (`body.past #world`).
...
- **Demo CSS:** the setting switch is `.tog` in the reference demo `style.css`; its track, thumb, on state and focus style belong to this recipe. The web Settings Recognition section currently uses `Switcher` at `Settings.tsx:170-176`.
