...
export const ICONS = [
/* ============================== TOOLS ============================== */
{ name: 'select', cat: 'Tools', label: 'Select', hover: '', press: '', body: '' },
...
for (const file of readdirSync(new URL('./acts/', import.meta.url)).filter((f) => f.endsWith('.mjs') && (!only || only.includes(f.slice(0, -4)))).sort()) {
  ...
  Object.assign(ic, { body: act.body, defs: act.defs, study: act.study, shape: act.shape ?? ic.shape, hover: act.study.caption, press: 'plays the same act' });
  delete ic.base;
  delete ic.mo;
}
