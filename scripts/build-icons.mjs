...
import { T16 } from '../icons/src/tuned16.mjs';
...
// ---------- static bake (identical to the Kamui builder) ----------
...
function staticSvg(ic, sw = SW, body = ic.body) {
...
`;
emit('src/icons/catalog.generated.ts', catalog);
...
`;
emit('src/icons/icons.generated.css', iconsCss);
...
`;
emit('src/icons/components.generated.tsx', components);

emit('public/icons.json', JSON.stringify({
...
for (const { ic, t16 } of entries) {
  emit(`public/icons/svg/${ic.name}.svg`, staticSvg(ic));
  emit(`public/icons/svg/16/${ic.name}.svg`, staticSvg(ic, t16?.sw ?? SW16, t16?.body ?? ic.body));
  emit(`public/icons/svg-animated/${ic.name}.svg`, animatedSvg(ic));
}
