...
const ST = T.status;
const ST_KEYS = Object.keys(ST).filter((k) => !k.startsWith('$') && k !== 'gestures');
...
// Lamp gestures (status.gestures): one keyframe track per gesture on the lit lamp's glow.
const GESTURES = Object.entries(T.status.gestures).filter(([k]) => !k.startsWith('$') && k !== 'dim' && T.status.gestures[k].ms > 0);
const DIM = T.status.gestures.dim;
const lampLevel = (l) => `brightness(${+(DIM.brightness + (1 - DIM.brightness) * l).toFixed(3)}) saturate(${+(DIM.saturate + (1 - DIM.saturate) * l).toFixed(3)})`;
const LAMP_GESTURES = GESTURES.map(([k, g]) => `@keyframes mu-led-${k} {\n${g.keys.map(([t, l]) => `  ${+(t * 100).toFixed(2)}% { filter: ${lampLevel(l)}; }`).join('\n')}\n}\n@utility animate-led-${k} {\n  animation: mu-led-${k} ${g.ms}ms ${g.ease ?? 'linear'} ${g.loop ? 'infinite' : 'both'};\n}`).join('\n');
...
/* Lamp gestures (status.gestures): how an LED behaves over time. */
${LAMP_GESTURES}
`;
...
emit('swift/Sources/MetalUI/Tokens/MetalLampGestures.generated.swift', `// Generated by scripts/build-tokens.mjs from tokens/tokens.json (status.gestures). Do not edit.
...
${allGestures.map(([k]) => `    case ${k}`).join('\n')}

    /// Seconds for one run (0: holds).
    public var duration: Double { switch self { ${allGestures.map(([k, g]) => `case .${k}: return ${num(g.ms / 1000)}`).join('; ')} } }
    public var loops: Bool { switch self { ${allGestures.map(([k, g]) => `case .${k}: return ${!!g.loop}`).join('; ')} } }
    /// (progress 0–1, level 0–1)
    public var keys: [(Double, Double)] { switch self { ${allGestures.map(([k, g]) => `case .${k}: return [${g.keys.map(([t, l]) => `(${num(t)}, ${num(l)})`).join(', ')}]`).join('; ')} } }
    /// Eased in and out between keys (breathe, rise), or linear.
    public var eased: Bool { switch self { ${allGestures.map(([k, g]) => `case .${k}: return ${!!g.ease}`).join('; ')} } }
}

public enum MetalLampDim {
    public static let brightness: Double = ${num(DIM.brightness)}
    public static let saturate: Double = ${num(DIM.saturate)}
}
`);

// ---------- sound (tokens.sound): the material and beeper recipes both engines play ----------
const S = T.sound;
const strip = (v) => Array.isArray(v) ? v.map(strip)
  ...
  : v;
const SOUND_MATERIALS = Object.keys(S.materials).filter((k) => !k.startsWith('$'));
const EARCONS = Object.keys(S.beeper.earcons).filter((k) => !k.startsWith('$'));
emit('packages/metalui/src/sound/recipes.generated.ts', `// Generated by scripts/build-tokens.mjs from tokens/tokens.json (sound). Do not edit.
// The recipes the Web Audio engine plays; MetalSound.generated.swift carries the same table.
export const SOUND = ${JSON.stringify(strip(S), null, 2)} as const;
export type SoundMaterial = ${SOUND_MATERIALS.map((m) => `'${m}'`).join(' | ')};
export type Earcon = ${EARCONS.map((e) => `'${e}'`).join(' | ')};
export type Reach = ${Object.keys(S.reach).filter((k) => !k.startsWith('$')).map((r) => `'${r}'`).join(' | ')};
export const SOUND_MATERIALS: readonly SoundMaterial[] = [${SOUND_MATERIALS.map((m) => `'${m}'`).join(', ')}];
export const EARCONS: readonly Earcon[] = [${EARCONS.map((e) => `'${e}'`).join(', ')}];
`);

const opt = (v) => (v === undefined ? 'nil' : num(v));
const swiftMode = ([ratio, gain, ms, glideTo, glideMs]) => `.init(ratio: ${num(ratio)}, gain: ${num(gain)}, decayMs: ${num(ms)}, glideTo: ${opt(glideTo)}, glideMs: ${opt(glideMs)})`;
const swiftNoise = (n) => `.init(kind: .${n.type}, frequency: ${num(n.f)}, q: ${opt(n.q)}, ms: ${num(n.ms)}, gain: ${num(n.gain)})`;
const swiftMaterial = ([k, m]) => `        case .${k}: // ${m.hear}
            return MetalMaterialSound(
                f0x: ${num(m.f0x)}, tone: ${num(m.tone)}, loud: ${num(m.loud)}, attackMs: ${num(m['attack-ms'])}, maxMs: ${num(m['max-ms'])},
                noisyModes: ${!!m['noisy-modes']}, thump: ${m.thump !== false},
                modes: [${m.modes.map(swiftMode).join(', ')}],
                noise: [${m.noise.map(swiftNoise).join(', ')}],
                grit: ${m.grit ? `.init(count: ${m.grit.n}, spreadMs: ${num(m.grit['spread-ms'])}, frequency: ${num(m.grit.f)}, level: ${num(m.grit.level)})` : 'nil'},
                bounce: ${m.bounce ? `.init(ms: ${num(m.bounce.ms)}, level: ${num(m.bounce.level)})` : 'nil'}
            )`;
const soundSwift = `// Generated by scripts/build-tokens.mjs from tokens/tokens.json (sound). Do not edit.
...
${SOUND_MATERIALS.map((m) => `    case ${m}`).join('\n')}

    public var recipe: MetalMaterialSound {
        switch self {
${Object.entries(S.materials).filter(([k]) => !k.startsWith('$')).map(swiftMaterial).join('\n')}
        }
    }
}
...
${Object.keys(S.reach).filter((k) => !k.startsWith('$')).map((r) => `    case ${r}`).join('\n')}

    public var send: Double { switch self { ${Object.entries(S.reach).filter(([k]) => !k.startsWith('$')).map(([r, v]) => `case .${r}: return ${num(v.send)}`).join('; ')} } }
...
${EARCONS.map((e) => `    case ${e}`).join('\n')}
    ...
    public var label: String { switch self { ${EARCONS.map((e) => `case .${e}: return ${JSON.stringify(S.beeper.earcons[e].label)}`).join('; ')} } }
    /// [midi, at ms, length ms, level]
    public var notes: [MetalEarconNote] {
        switch self {
${EARCONS.map((e) => `        case .${e}: return [${S.beeper.earcons[e].notes.map(([m, at, len, lv]) => `.init(midi: ${m}, atMs: ${num(at)}, lengthMs: ${num(len)}, level: ${num(lv)})`).join(', ')}]`).join('\n')}
        }
    }
    public var levelDb: Double { switch self { ${EARCONS.map((e) => `case .${e}: return ${num(S.policy['level-db'][S.beeper.earcons[e].level ?? 'beep'])}`).join('; ')} } }
}

/// The rest of the sound foundation: policy, pitch, room, thump and the beeper's timbre.
public enum MetalSoundTokens {
    public static let gapMs: Double = ${num(S.policy.rate['gap-ms'])}
    public static let burst: Int = ${S.policy.rate.burst}
    public static let burstWindowMs: Double = ${num(S.policy.rate['burst-window-ms'])}
    public static let varyF0: Double = ${num(S.policy.vary.f0)}
    public static let varyDecay: Double = ${num(S.policy.vary.decay)}
    public static let varyFilter: Double = ${num(S.policy.vary.filter)}
    /// [plays, dB]: a part that has played this often in a session plays this much quieter.
    public static let decayAfter: [(plays: Int, db: Double)] = [${S.policy['decay-after'].map(([n, d]) => `(${n}, ${num(d)})`).join(', ')}]
    public static let actDb: Double = ${num(S.policy['level-db'].act)}
    public static let limitDb: Double = ${num(S.policy['level-db'].limit)}
    /// [rendered size in points, gain]
    public static let sizeGain: [(size: Double, gain: Double)] = [${S.policy['size-gain'].map(([s, g]) => `(${num(s)}, ${num(g)})`).join(', ')}]
    public static let pitchBase: Double = ${num(S.pitch.base)}
    public static let pitchRef: Double = ${num(S.pitch.ref)}
    public static let pitchHeavy: Double = ${num(S.pitch.heavy)}
    public static let bodySize: Double = ${num(S.pitch.body)}
    public static let roomSeconds: Double = ${num(S.room.seconds)}
    public static let roomCurve: Double = ${num(S.room.curve)}
    public static let thumpFrom: Double = ${num(S.thump.from)}
    public static let thumpTo: Double = ${num(S.thump.to)}
    public static let thumpMs: Double = ${num(S.thump.ms)}
    public static let thumpAbove: Double = ${num(S.thump.above)}
    public static let thumpLevel: Double = ${num(S.thump.level)}
    public static let contactAttackMs: Double = ${num(S.render['contact-attack-ms'])}
    public static let bandQ: Double = ${num(S.render['band-q'])}
    public static let bandGain: Double = ${num(S.render['band-gain'])}
    public static let gritQ: Double = ${num(S.render['grit-q'])}
    public static let gritMs: Double = ${num(S.render['grit-ms'])}
    public static let gritSpread: Double = ${num(S.render['grit-spread'])}
    public static let bouncePitch: Double = ${num(S.render['bounce-pitch'])}
    public static let ceilingHz: Double = ${num(S.render['ceiling-hz'])}
    public static let beeperResonance: Double = ${num(S.beeper.resonance)}
    public static let beeperQ: Double = ${num(S.beeper.q)}
