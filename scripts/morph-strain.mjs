...
// ---------- the life set's declared pairs (Kamui import plan §3.5) ----------
// The pairs a product switches between: the time of day, the weather, sleep, and the feelings that an
// edit, a correction or a Jev re-ask can turn into each other. Budget: under 1.
export const LIFE_PAIRS = [
  ['morning', 'afternoon'], ['afternoon', 'evening'], ['evening', 'late-night'],
  ['sunny', 'hot'], ['sunny', 'rain'], ['rain', 'cold'],
  ['sleep', 'nap'], ['nap', 'bad-night'], ['tired', 'rested'],
  ['calm', 'sad'], ['calm', 'happy'], ['happy', 'excited'], ['sad', 'dull'], ['dull', 'bored'],
  ['focused', 'distracted'], ['distracted', 'bored'], ['bored', 'lonely'],
  ['drained', 'grateful'], ['grateful', 'overwhelmed'], ['anxious', 'angry'], ['angry', 'frustrated'],
];
...
if (flag('--life')) {
  ...
  for (const g of LIFE_GLYPHS) {
    try { const r = glyphParts(g, lifeStaticSvg); if (isMorphable(r)) rows[g.name] = r; else out.push(`${g.name} (solid plate: K0)`); } catch (e) { out.push(`${g.name} (unreadable)`); }
  }
  const F = frames(rows);
  const both = LIFE_PAIRS.flatMap(([a, b]) => [[a, b], [b, a]]);
  const scored = both.map(([a, b]) => ({ a, b, s: F[a] && F[b] ? planFrames(F[a], F[b]).strain : null }));
  const lines = [`## Declared life pairs (budget: under 1)
`, table(['pair', 'strain', 'travel (u)', 'lone share', 'topology share', 'in budget'], scored.map(({ a, b, s }) => (s ? [`${a} → ${b}`, f2(s.total), f2(s.travel), f2(s.lone), f2(s.topology), s.total < 1 ? 'yes' : 'no'] : [`${a} → ${b}`, 'out of family', '', '', '', 'no'])))];
  const inBudget = scored.filter((x) => x.s && x.s.total < 1).length;
  lines.push(`
${inBudget} of ${scored.length} ordered pairs are under 1. Out of the morph family: ${out.join(', ') || 'none'}.`);
  const text = lines.join('\n');
  console.log(text);
  const outDir = arg('--out');
  if (outDir) {
    mkdirSync(resolve(outDir), { recursive: true });
    writeFileSync(join(resolve(outDir), 'LIFE-STRAIN.md'), `# Life pairs: morph strain\n\nGenerated by \`node scripts/morph-strain.mjs --life --out ${outDir}\`. Strain is the engine's own score (docs/MORPH.md): under 1 a pair reads as one object changing; 1–2 is an honest transformation; 2 and over should ride the drum or be redrawn under the grammar.\n\n${text}\n`);
    writeFileSync(join(resolve(outDir), 'life-filmstrips.html'), PAGE('Life pairs: filmstrips', LIFE_PAIRS.filter(([a, b]) => F[a] && F[b]).map(([a, b], i) => filmstrip(a, b, F[a], F[b], `l${i}`)).join('')));
  }
} else if (!proposalsPath) {
