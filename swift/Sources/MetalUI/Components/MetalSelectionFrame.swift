...
// KAMUI-14 · Selection frame. Mirrors components/selection-frame: every value is MetalPresence
...
/// Which part of KAMUI-14 is showing.
...
private struct MetalSelectionFrameModifier: ViewModifier {
    ...
    private func ring(_ size: CGSize) -> some View {
        ...
        .opacity(state == .selected && (entered || variant == .lite) ? 1 : 0)
    ...
    private func edgeLight(_ edge: MetalSelectionEdge, _ size: CGSize) -> some View {
        ...
        let fade = Gradient(stops: [.init(color: light.opacity(0), location: 0), .init(color: light, location: 0.22), .init(color: light, location: 0.78), .init(color: light.opacity(0), location: 1)])
        return Rectangle()
    ...
    private func handleViews(_ size: CGSize) -> some View {
        ForEach(MetalSelectionHandle.allCases, id: \.self) { handle in
            ...
            .opacity(entered || variant == .lite ? 1 : 0)
...
extension View {
    /// KAMUI-14 around this view: the one selection for every kind of object.
    ///
    ...
    public func metalSelectionFrame(
        ...
        modifier(MetalSelectionFrameModifier(
            ...
            count: count, copied: copied, edge: edge, onHandleDrag: onHandleDrag
        ))
    }
}

/// A standalone KAMUI-14 frame of a given size, for overlays that draw it apart from the object
/// (Kamui's AppKit presence overlay hosts this, or reads `MetalPresence` directly).
public struct MetalSelectionFrame: View {
    ...
    public init(size: CGSize, state: MetalSelectionState = .selected, variant: MetalSelectionVariant = .ring, mode: MetalSelectionMode = .idle, radius: CGFloat = 0, handles: MetalSelectionHandles = .object, count: Int? = nil) {
        self.size = size
        self.state = state
    ...
    public var body: some View {
        Color.clear
            ...
            .metalSelectionFrame(state, variant: variant, mode: mode, radius: radius, handles: handles, count: count)
    }
}
